...
- 다이얼로그/메뉴/상태바 등 사용자 노출 문자열은 렌더링 코드에 하드코딩하지 않는다.
- UI 텍스트는 `src/ui/i18n.rs`의 `TextKey`/`MessageKey`를 통해 조회한다.
- 새 `InputPurpose`/다이얼로그 추가 시 영어/한국어 번역 쌍을 반드시 함께 추가한다.
- 한국어 렌더링 회귀를 막기 위해, 가능하면 렌더 테스트에서 영어 하드코딩 문구 미노출을 검증한다.
