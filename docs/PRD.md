...
**현재 Phase**: Phase 3 완료 (3.1 ✅, 3.2 ✅, 3.3 ✅, 3.4 ✅)
**다음 Phase**: Phase 4 - 단축키 시스템 리팩토링 (Vim 스타일)
**전체 진행률**: 3/10 Phases 완료 (30%)
...
1. Vim 스타일 키바인딩 추가 (j/k/h/l, y/x/d/r/a 등)
2. 키 시퀀스 시스템 (gg 등)
3. 단축키 도움말 팝업 (`?`)
4. 커맨드 바 업데이트
...
**작업 항목**:
- [ ] Vim 네비게이션 키 추가 (j/k/h/l/G)
- [ ] Vim 파일 조작 키 추가 (y/x/d/D/a/r/i)
- [ ] Vim 선택 키 변경 (v, u)
- [ ] `Ctrl+D` 용도 변경 (선택 해제 → 반 페이지 아래로)
- [ ] `Ctrl+U` 반 페이지 위로 추가
- [ ] `Ctrl+R` 새로고침 추가
- [ ] 기존 `*`, `+` 제거
...
**작업 항목**:
- [ ] `pending_key` 상태 구현 (App 구조체에 `Option<char>` 필드)
- [ ] `g` 키 입력 시 pending 상태 진입
- [ ] pending 상태에서 `g` 입력 시 `gg` 실행 (목록 맨 위로)
- [ ] pending 상태에서 다른 키 또는 타임아웃(~500ms) 시 pending 해제
- [ ] 상태바에 pending 키 표시 (예: `g_`)
...
- [ ] `DialogKind::Help` 추가
- [ ] `?` 키로 도움말 다이얼로그 열기
- [ ] 상황별 키바인딩 표시 (Normal / Dialog / Menu 모드)
- [ ] 카테고리별 정리 (탐색, 파일 조작, 선택, 시스템)
- [ ] 스크롤 지원 (j/k 또는 ↑/↓)
- [ ] Esc/q/? 로 닫기
...
**작업 항목**:
- [ ] 기존 F키 전용 표시에서 Vim 키 병행 표시로 변경
- [ ] 표시 형식: `y:Copy x:Move d:Del a:New r:Ren ?:Help`
- [ ] 공간 부족 시 축약 표시
...
#### 7.2 파일 비교
**작업 항목**:
- [ ] Diff 알고리즘 구현 or 라이브러리 사용
- [ ] 좌우 비교 UI
- [ ] 차이점 하이라이트

#### 7.3 기타 뷰어
**작업 항목**:
- [ ] 이미지 미리보기 (터미널 이미지 프로토콜)
- [ ] PDF 정보 표시
- [ ] 파일 속성 상세 보기
...
#### 8.2 터미널 에디터 통합
**작업 항목**:
- [ ] TUI raw mode 해제/재활성화
- [ ] 에디터 실행 (vi, vim, nano, emacs)
- [ ] 에디터 종료 후 복귀
- [ ] 기본 에디터 설정
